<template>
  ...
  <div class="answer-form" v-if="showAnswerForm">
    ...
    <button @click="saveAnswer" class="save-answer-button">
      Salvar Resposta
    </button>
  </div>
  ...
  setup() {
    ...
    const saveAnswer = async () => {
      ...
      const response = await axios.post(
        `http://localhost:3030/api/posts/${post.value.id}/answers`,
        newAnswer,
      );
    ...
    onMounted(async () => {
      ...
      const answersResponse = await axios.get(
        `http://localhost:3030/api/posts/${postId}/answers`,
      );
    ...
    const removeAnswer = async (answerId: number) => {
      const response = await axios.delete(
        `http://localhost:3030/api/answers/${answerId}`,
      );
