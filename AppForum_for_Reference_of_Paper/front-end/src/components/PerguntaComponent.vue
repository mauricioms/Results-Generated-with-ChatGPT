...
const loadRespostas = async (_id: number) => {
  const response = await fetch('/app-forum/api/pergunta/' + _id)
...
function salvarReply() {
  ...
  sendResposta(resposta.value).then(async () => {
...
<template>
  <div class="card mb-4">
    ...
    <div class="card-body" v-for="r in pergunta.respostas" :key="r.id">
    ...
    <div class="card-footer d-flex flex-wrap justify-content-between align-items-center px-0 pt-0 pb-3">
      <div class="px-4 pt-3">
        <a type="button" class="btn btn-default" href="/app-forum/local/#/"> <i class="bi bi-reply"></i>&nbsp; Go Back </a>
      </div>
      <div class="px-4 pt-3">
        <button type="button" class="btn btn-primary" @click="addReply(pergunta.id)"><i class="bi bi-reply"></i>&nbsp; Answer</button>
      </div>
    </div>
  </div>
</template>
